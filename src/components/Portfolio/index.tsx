    ...
    return (
        ...
        >
            <motion.div 
                ...
                // 🚀 FIX: Adjusted max-height and flex properties for mobile scrolling
...
const HorizontalProject = ({ project, xOffset, onDecrypt, numParallax, isTouchDevice }: { project: any, xOffset: any, onDecrypt: () => void, numParallax: any, isTouchDevice: boolean }) => {
  const Blueprint = blueprintMap[project.category];

  return (
    // 🚀 FIX: h-[100dvh] handles Safari mobile URL bar dynamic height beautifully
    <div className="w-screen h-[100dvh] flex-shrink-0 flex items-center justify-center relative p-4 sm:p-8 lg:p-24 overflow-hidden pt-24 lg:pt-24">
      ...
      <div className="w-full max-w-7xl h-full max-h-[85dvh] lg:max-h-[800px] flex flex-col lg:flex-row rounded-[24px] lg:rounded-[40px] bg-[#0A0C10]/60 backdrop-blur-3xl border border-white/10 shadow-[0_30px_100px_rgba(0,0,0,0.8)] overflow-hidden relative z-10 texture-noise group">
        ...
        <div className="w-full lg:w-5/12 p-6 sm:p-10 lg:p-16 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-white/5 relative z-20 flex-1 overflow-y-auto custom-scrollbar">
          <div>
            <div className="flex items-center gap-3 mb-6 lg:mb-8 mt-2 lg:mt-0">
                ...
                <span className="font-mono text-[9px] sm:text-[10px] tracking-[0.2em] uppercase text-white/50">{project.category}</span>
            </div>
            
            <h2 className="text-3xl sm:text-4xl lg:text-6xl font-black text-white tracking-tighter leading-[1.05] mb-4 lg:mb-6">
                {project.title}
            </h2>
            <p className="text-gray-400 text-sm sm:text-base lg:text-lg font-light leading-relaxed mb-6 lg:mb-8">
                {project.description}
            </p>

            <div className="flex flex-wrap gap-2">
                {project.tech.map((t: string, i: number) => (
                    <span key={i} className="px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg bg-white/5 border border-white/10 font-mono text-[8px] sm:text-[9px] tracking-widest text-white/70 uppercase shadow-inner">
                        {t}
                    </span>
                ))}
            </div>
        ...
        {/* 🚀 FIX: min-h on mobile prevents it from collapsing if left text is too long */}
...
const PortfolioSection = () => {
  ...
  // Map smooth velocity to a slight default 1 when not moving (disabled on mobile for smoother scroll)
      ...
      <div className="w-full bg-[#030406]">
        ...
        <section ref={targetRef} className="relative w-full h-[400vh]">
          <div className="sticky top-0 h-[100dvh] w-full overflow-hidden flex items-center bg-[#030406]">
            ...
            <motion.div 
                style={{ x, scale: isTouchDevice ? 1 : springTrackScale }} 
            ...
            {/* Progress Bar - Repositioned slightly for mobile */}
